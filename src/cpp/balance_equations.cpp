...
namespace balance_equations {
    ...
    int compute_internal_force(const double (&dNdX)[3], const variableVector &F, const variableVector &PK2,
                               double (&fint)[3]) {
        /*!
    ...
    int compute_internal_force(const unsigned int &i, const double (&dNdX)[3], const variableVector &F,
                               const variableVector &PK2, double &fint_i) {
        /*!
         * Compute the internal force given the gradient of the shape function in the reference configuration,
    ...
    void compute_body_force(const unsigned int &i, const double &N, const double &density, const double (&b)[3],
                            double &fb_i) {
        /*!
    ...
    int compute_internal_couple(const double &N, const double (&dNdX)[3], const variableVector &F,
                                const variableVector &chi, const variableVector &PK2, const variableVector &SIGMA,
                                const variableVector &M, double (&cint)[9]) {
        /*!
         ...
         *     [ cint_{ 11 }, cint_{ 12 }, cint_{ 13 }, cint_{ 21 }, cint_{ 22 }, cint_{ 23 }, cint_{ 31 }, cint_{ 32 },
         * cint_{ 33 } ]
         */
    ...
    int compute_internal_couple(const unsigned int &i, const unsigned int &j, const double &N, const double (&dNdX)[3],
                                const variableVector &F, const variableVector &chi, const variableVector &PK2,
                                const variableVector &SIGMA, const variableVector &M, double &cint_ij) {
        /*!
    ...
    void compute_body_couple(const unsigned int &i, const unsigned int &j, const double &N, const double &density,
                             const double (&l)[9], double &cb_ij) {
        /*!
    ...
    void compute_inertia_couple(const double &N, const double &density, const double (&omega)[9],
                                double (&cinertia)[9]) {
        /*!
    ...
    void compute_inertia_couple(const unsigned int &i, const unsigned int &j, const double &N, const double &density,
                                const double (&omega)[9], double &cinertia_ij) {
        /*!
    ...
    int compute_inertia_couple(const double &N, const double &density, const double (&chi)[9],
                               const double (&D2ChiDt2)[9], const double (&referenceInertia)[9],
                               double (&cinertia)[9]) {
        /*!
    ...
    int compute_internal_force_jacobian(const double &N, const double (&dNdX)[3], const double &eta,
                                        const double (&detadX)[3], const variableVector &F, const variableVector &PK2,
                                        const variableMatrix &DPK2Dgrad_u, const variableMatrix &DPK2Dphi,
                                        const variableMatrix &DPK2Dgrad_phi, variableMatrix &DfintDU) {
        /*!
         * Compute the jacobian of the internal force
         *
         * fint_i = - N_{ , I } PK2_{ I J } F_{ i J }
         *
         * returns 0 if there are no errors, 1 if F is not the right size, 2 if PK2 is not the right size, 3 if
         * DPK2Dgrad_u is not the right size, 4 if DPK2Dphi is not the right size, and 5 if DPK2Dgrad_phi is not the
         * right size.
         *
         * :param const double &N: The shape function value
         * :param const double ( &dNdX )[ 3 ]: The gradient of the shape function w.r.t. X in the reference
         * configuration. :param const double &eta: The interpolation function value :param const double ( &detadX )[ 3
         * ]: The gradient of the interpolation function w.r.t. X in the reference configuration. :param const
         * variableVector &F: The deformation gradient. :param const variableVector &PK2: The second Piola Kirchoff
         * stress. :param const variableMatrix &DPK2Dgrad_u: The Jacobian of the second Piola Kirchoff stress w.r.t. the
         * gradient of the macro displacement w.r.t. X in the reference configuration. :param const variableVector
         * &DPK2Dphi: The Jacobian of the second Piola Kirchoff stress w.r.t. the micro displacement. :param const
         * variableMatrix &DPK2Dgrad_phi: The Jacobian of the second Piola Kirchoff stress w.r.t. the gradient of the
         * micro displacement. :param variableMatrix DfintDU: The Jacobian of the internal force w.r.t. the degree of
         * freedom vector which is organized: [ u1, u2, u3, phi_11, phi_12, phi_13, phi_21, phi_22, phi_23, phi_31,
         * phi_32, phi_33 ]
         */
        ...
        for (unsigned int i = 0; i < dim; i++) {
            for (unsigned int j = 0; j < NDOF; j++) {
                errorCode = compute_internal_force_jacobian(i, j, N, dNdX, eta, detadX, F, PK2, DPK2Dgrad_u, DPK2Dphi,
                                                            DPK2Dgrad_phi, DfintDU[i][j]);
    ...
    int compute_internal_force_jacobian(const unsigned int &i, const unsigned int &j, const double &N,
                                        const double (&dNdX)[3], const double &eta, const double (&detadX)[3],
                                        const variableVector &F, const variableVector &PK2,
                                        const variableMatrix &DPK2Dgrad_u, const variableMatrix &DPK2Dphi,
                                        const variableMatrix &DPK2Dgrad_phi, variableType &DfintDU_ij) {
        /*!
         * Compute the jacobian of the internal force
         *
         * fint_i = - N_{ , I } PK2_{ I J } F_{ i J }
         *
         * returns 0 if there are no errors, 1 if F is not the right size, 2 if PK2 is not the right size, 3 if
         * DPK2Dgrad_u is not the right size, 4 if DPK2Dphi is not the right size, 5 if DPK2Dgrad_phi is not the right
         * size, 6 if i is out of range, 7 if j is out of range.
         *
         ...
         * &DPK2Dphi: The Jacobian of the second Piola Kirchoff stress w.r.t. the micro displacement. :param const
         * variableMatrix &DPK2Dgrad_phi: The Jacobian of the second Piola Kirchoff stress w.r.t. the gradient of the
         * micro displacement. :param variableType DfintDU_ij: The ij'th component of the internal force jacobian w.r.t.
         * the degree of freedom vector which is organized: [ u1, u2, u3, phi_11, phi_12, phi_13, phi_21, phi_22,
         * phi_23, phi_31, phi_32, phi_33 ] where it's position in this vector is j
         */
        ...
        if (j < 3) {
            ...
            for (unsigned int I = 0; I < dim; I++) {
                for (unsigned int J = 0; J < dim; J++) {
                    ...
                    for (unsigned int K = 0; K < dim; K++) {
                        DfintDU_ij -=
                            dNdX[I] * DPK2Dgrad_phi[dim * I + J][dim * (j - 3) + K] * detadX[K] * F[dim * i + J];
                    }
                }
            }
        }
    ...
    int compute_internal_couple_jacobian(const double &N, const double (&dNdX)[3], const double &eta,
                                         const double (&detadX)[3], const variableVector &F, const variableVector &chi,
                                         const variableVector &PK2, const variableVector &SIGMA,
                                         const variableVector &M, const variableMatrix &DPK2Dgrad_u,
                                         const variableMatrix &DPK2Dphi, const variableMatrix &DPK2Dgrad_phi,
                                         const variableMatrix &DSIGMADgrad_u, const variableMatrix &DSIGMADphi,
                                         const variableMatrix &DSIGMADgrad_phi, const variableMatrix &DMDgrad_u,
                                         const variableMatrix &DMDphi, const variableMatrix &DMDgrad_phi,
         ...
         * :param const double ( &detadX )[ 3 ]: The gradient of the interpolation function w.r.t. the reference
         * coordinates. :param const variableVector &F: The deformation gradient. :param const variableVector &chi: The
         * micro deformation tensor. :param const variableVector &PK2: The second Piola Kirchoff stress tensor. :param
         * const variableVector &SIGMA: The symmetric micro stress tensor in the reference configuration. :param const
         * variableVector &M: The higher order stress tensor in the reference configuration. :param const variableMatrix
         * &DPK2Dgrad_u: The derivative of the PK2 stress w.r.t. the gradient of the macro displacement w.r.t. the
         * reference configuration. :param const variableMatrix &DPK2Dphi: The derivative of the PK2 stress w.r.t. the
         * micro displacement. :param const variableMatrix &DPK2Dgrad_phi: The derivative of the PK2 stress w.r.t. the
         * gradient of the micro displacement w.r.t. the reference configuration. :param const variableMatrix
         * &DSIGMADgrad_u: The derivative of the symmetric micro stress in the reference configuration w.r.t. the
         * gradient of the macro displacement w.r.t. the reference configuration. :param const variableMatrix
         * &DSIGMADphi: The derivative of the symmetric micro stress in the reference configuration w.r.t. the micro
         * displacement. :param const variableMatrix &DSIGMADgrad_phi: The derivative of the symmetric micro stress in
         * the reference configuration w.r.t. the gradient of the micro displacement w.r.t. the reference configuration.
        ...
        for (unsigned int i = 0; i < dim * dim; i++) {
            for (unsigned int j = 0; j < NDOF; j++) {
                errorCode =
                    compute_internal_couple_jacobian(i, j, N, dNdX, eta, detadX, F, chi, PK2, SIGMA, M, DPK2Dgrad_u,
                                                     DPK2Dphi, DPK2Dgrad_phi, DSIGMADgrad_u, DSIGMADphi,
                                                     DSIGMADgrad_phi, DMDgrad_u, DMDphi, DMDgrad_phi, DcintDU[i][j]);
    ...
    int compute_internal_couple_jacobian(const unsigned int &i, const unsigned int &j, const double &N,
                                         const double (&dNdX)[3], const double &eta, const double (&detadX)[3],
                                         const variableVector &F, const variableVector &chi, const variableVector &PK2,
                                         const variableVector &SIGMA, const variableVector &M,
                                         const variableMatrix &DPK2Dgrad_u, const variableMatrix &DPK2Dphi,
                                         const variableMatrix &DPK2Dgrad_phi, const variableMatrix &DSIGMADgrad_u,
                                         const variableMatrix &DSIGMADphi, const variableMatrix &DSIGMADgrad_phi,
                                         const variableMatrix &DMDgrad_u, const variableMatrix &DMDphi,
                                         const variableMatrix &DMDgrad_phi, variableType &DcintDU_ij) {
        /*!
         * Compute the jacobian of the internal couple
         * cint_{ ij } = N F_{ iI } ( PK2_{ JI } - SIGMA_{ JI } ) F_{ jJ } - N_{ ,K } F_{ iI } \chi_{ jJ } M_{ KIJ }
         *
        ...
        if (j < 3) {
            for (unsigned int I = 0; I < dim; I++) {
                for (unsigned int J = 0; J < dim; J++) {
                    for (unsigned int K = 0; K < dim; K++) {
                        DcintDU_ij +=
                            N * F[dim * (i / 3) + I] *
                            (DPK2Dgrad_u[dim * J + I][dim * j + K] - DSIGMADgrad_u[dim * J + I][dim * j + K]) *
            ...
            for (unsigned int I = 0; I < dim; I++) {
                for (unsigned int J = 0; J < dim; J++) {
                    DcintDU_ij += N * F[dim * (i / 3) + I] *
                                  (DPK2Dphi[dim * J + I][j - 3] - DSIGMADphi[dim * J + I][j - 3]) * eta *
                                  F[dim * (i % 3) + J];
                    for (unsigned int K = 0; K < dim; K++) {
                        DcintDU_ij += N * F[dim * (i / 3) + I] *
                                      (DPK2Dgrad_phi[dim * J + I][dim * (j - 3) + K] -
                        ...
                        for (unsigned int L = 0; L < dim; L++) {
                            DcintDU_ij -= dNdX[K] * F[dim * (i / 3) + I] * chi[dim * (i % 3) + J] *
                                          DMDgrad_phi[dim * dim * K + dim * I + J][dim * (j - 3) + L] * detadX[L];
                        }
    ...
    int compute_inertia_couple_jacobian(const double &N, const double &eta, const double &density,
                                        const double (&chi)[9], const double (&D2ChiDt2)[9],
                                        const variableMatrix &D3ChiDt2DChi, const double (&referenceInertia)[9],
                                        variableMatrix       &DcinertiaDU) {
        /*!
         ...
         * :param variableMatrix &DcinertiaDU: The Jacobian of the inertia couple term w.r.t. the degree of freedom
         * vector
         */
        ...
        for (unsigned int i = 0; i < dim * dim; i++) {
            for (unsigned int j = 0; j < dim * dim; j++) {
                ...
                errorCode = compute_inertia_couple_jacobian(i, j, N, eta, density, chi, D2ChiDt2, D3ChiDt2DChi_j,
                                                            referenceInertia, DcinertiaDU[i][j]);
    ...
    int compute_inertia_couple_jacobian(const unsigned int &i, const unsigned int &j, const double &N,
                                        const double &eta, const double &density, const double (&chi)[9],
        ...
        for (unsigned int K = 0; K < dim; K++) {
            for (unsigned int L = 0; L < dim; L++) {
                DcinertiaDU_ij -=
                    N * eta * density * referenceInertia[dim * K + L] * D3ChiDt2DChi_j[dim * k + K] * chi[dim * l + L];
    ...
    int compute_inertia_force_jacobian(const unsigned int &i, const unsigned int &j, const double &N, const double &eta,
                                       const double &density, const double (&a)[3], const variableVector &DaDu_i,
                                       variableType &DfinertiaDU_ij) {
        /*!
